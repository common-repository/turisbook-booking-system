<?php
if ( ! function_exists('turisbook_estab_post_type') ) {

	function turisbook_estab_post_type() {

		$type_name = __( 'Establishment', 'turisbook-booking-system' );
		$type_name_plural = __( 'Establishments', 'turisbook-booking-system' );
		$slug = 'hotel';
		...
		register_post_type( 'turisbook_estab', $args );

	}
	add_action( 'init', 'turisbook_estab_post_type', 0 );

	add_filter('template_include', 'Tbs_turisbook_single_establishment_template');
	function Tbs_turisbook_single_establishment_template( $template ) {
		if ( is_singular('turisbook_estab') ) {
			$theme_files = array('single-turisbook_establishment.php');
			$exists_in_theme = locate_template($theme_files, false);
			if ( $exists_in_theme != '' ) {
				return $exists_in_theme;
			} else {
				return TURISBOOK_PUBLIC_PATH . '/partials/single-turisbook_establishment.php';
			}
		}
		return $template;
	}
	...
	add_filter('template_include', 'Tbs_turisbook_establishment_template');

	function Tbs_turisbook_establishment_template( $template ) {
		if ( is_post_type_archive('turisbook_estab') ) {
			$theme_files = array('archive-turisbook_establishment.php');
			$exists_in_theme = locate_template($theme_files, false);
			if ( $exists_in_theme != '' ) {
				return $exists_in_theme;
			} else {
				return TURISBOOK_PUBLIC_PATH . '/partials/archive-turisbook_establishment.php';
			}
		}
		return $template;
	}
	
	add_action('parse_query', 'turisbook_establishments_sort_posts');

	function turisbook_establishments_sort_posts($q)
	{
		if(!$q->is_main_query() || is_admin())
			return;

		if(!is_post_type_archive('turisbook_estab')) return;
	...
	add_action( 'pre_get_posts', 'turisbook_establishments_nr_posts' );

	function turisbook_establishments_nr_posts( $query ) {
		if ( !is_admin() && $query->is_main_query() && is_post_type_archive('turisbook_estab') ) {
